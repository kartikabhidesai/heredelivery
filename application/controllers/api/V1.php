     ...
     $this->form_validation->set_rules('device_token', 'device_token', 'required');
        if($this->form_validation->run() == TRUE)
        {
         ...
         if($this->form_validation->run() == TRUE)
         {
            ...
            if($this->form_validation->run() == TRUE)
            {
                  $arr=array("username"=>$this->input->post('username'),"frist_name"=>$this->input->post('frist_name'),"last_name"=>$this->input->post('last_name'),
                  "phone_number"=>$this->input->post('phone_number')?:'', "email"=>$this->input->post('email'),
                  "password"=>sha1($this->input->post('password')), "created_on"=>date("Y-m-d H:i:s"),"device_token"=>$this->input->post('device_token'));
